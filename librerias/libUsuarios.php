<?php
	...
	switch ($_POST["type"]) {
		case 1:
			...
			$rSQlusuarios = mysqli_query($conexion, "SELECT * FROM sesion ");
			if (mysqli_num_rows($rSQlusuarios) > 0) {
				...
				while ($filaUsuarios = mysqli_fetch_assoc($rSQlusuarios)) { ?>
					<tr>
						...
						<td class="text-center text-capitalize"><?php echo $filaUsuarios["nombreUsuario"] ?></td>
						<td class="text-center text-capitalize"><?php echo $filaUsuarios["user"] ?></td>
						<td class="text-center text-capitalize">
							<div class="btn-group" role="group" aria-label="...">
						  		<button type="button" class="btn btn-info" onclick="modificarUsuario(this.id)" id="<?php echo $filaUsuarios["id_sesion"] ?>">
						  			<span class="fa fa-pencil-square-o"></span>
						  		</button>
						  		<button type="button" class="btn btn-warning" onclick="eliminarUsuario(this.id)" id="<?php echo $filaUsuarios["id_sesion"] ?>">
						  			<span class="fa fa-trash"></span>
						  		</button>
						  		<button type="button" class="btn btn-danger" onclick="cambiarContrasena(this.id)" id="<?php echo $filaUsuarios["id_sesion"] ?>">
